...
number: 1
name: Launch the app elevated. Navigate to Gaming Tweaks from both the Home card and the nav sidebar entry.
expected: |
  Page renders exactly 11 toggle rows (GPU HDCP Override, GPU P0 State, GPU MSI Mode, AMD Software Settings, Intel Graphics Settings, Device Manager Power Savings, Network Adapter Power Savings, Write Cache Buffer Flushing, Force IPv4-Only Networking, Gaming Power Plan, High-Precision Timer Resolution), 2 dropdowns (SvcHost Split Threshold, Win32 Priority Separation), 2 display-settings shortcut buttons, and the D-06 driver-tools button section (12 buttons). Akari OS Tweaks page still shows exactly 32 toggles, unaffected.
