...
- [x] **GAMING-01**: User can toggle gaming/latency/service tweaks (SvcHost split threshold, Win32 priority separation, service configuration dropdowns), with the same real-state and revert behavior as Tweaks (TWEAKS-01/TWEAKS-03)
...
| Full removal of Microsoft Store, WebView2, or Edge runtime dependencies | Most-cited cause of breakage in debloat post-mortems (breaks app installs, in-app browsers, unrelated apps that depend on WebView2) — debloat scope stays limited to user-facing bloat apps — **Partially overridden 2026-09-01** (Phase 3 discussion, D-06/D-07): the Debloat page's "Microsoft Edge — Remove" action explicitly includes full WebView2 runtime removal as an informed, accepted-risk exception for that one action; Microsoft Store removal remains fully out of scope. See `.planning/phases/03-debloat/03-CONTEXT.md` D-06/D-07. |
